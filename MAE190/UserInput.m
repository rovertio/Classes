...
MatData = readcell("MAE190_MaterialProps - Sheet1.csv");
...
mat_prop = struct;
    for i=1:size(database, 1)
        if isequal(name, string(database(i,1)))
            ...
            mat_prop.Sut_SI = cell2mat(database(i,2));
            mat_prop.Sut_Imperial = cell2mat(database(i,3));
            mat_prop.Sy_SI = cell2mat(database(i,4));
            mat_prop.Sy_Imperial = cell2mat(database(i,5));
        end
    end
end 
